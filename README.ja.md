...
- `xffuf`: ffufとctxのキャリブレーションでHTTP Virtual Hostを探索するアドオン

`xssh`、`xftp`、`xsmb` はctxのアドオンです。ctxのJSON APIを利用し、ctxのSQLiteデータベースは直接参照しません。
...
依存パッケージは各Debianパッケージで宣言しています。`xssh` は `openssh-client` と `sshpass`、`xftp` は `lftp`、`xsmb` は `smbclient`、`xgobuster` は `gobuster` と `wordlists`、`xffuf` は `ffuf` と `seclists` を使用します。
...
`xffuf vhost` はffufでHTTP Virtual Hostを探索します。ctxからHTTPサービスとドメインを選択し、ランダムなホスト名へキャリブレーションを行って、安定したレスポンスフィルターを提案します。通常実行では信頼基準を満たした場合にフィルターを自動適用します。`xffuf vhost --suggest` でキャリブレーション統計を表示した後、提案したフィルターを実際のワードリストで試走するか確認します。試走ではコマンドログ、検索済みキャッシュ、ワードリスト履歴、`xhost`登録を行いません。`xffuf vhost --trial -fw 125`で直接試走することもできます。`-fw`、`-fs`、`-fl`、`-fc`、`-fr` などのffufフィルターを手動指定できます。通常実行が完了した場合だけ結果を`xhost`へ登録し、ワードリストキャッシュを更新します。ヒット数が異常に多い場合は、自動登録とキャッシュ更新を保留します。

DNSサブドメイン探索は `xgobuster dns` で実行します。現在のターゲットに`xhost`で登録したホスト名が1件なら自動選択し、複数ある場合は選択します。プロンプトを避ける場合は `-d` または `--domain` でドメインを指定し、単発のワードリストを使う場合は `-w` または `--wordlist` を指定します。DNS探索はターゲットとドメインごとのキャッシュを持ち、`--status`で進捗を表示し、`--clear-cache`でDNSキャッシュだけを削除し、`--next`で次のワードリストへ進み、`--force`で最初のワードリストを再実行できます。既定のDNS問い合わせ数上限は`dns.max-queries`で設定し、既定値は10,000です。その他のGobuster DNSオプションもxgobusterのオプションの後ろへ渡せます。
