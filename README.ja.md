...
依存パッケージは各Debianパッケージで宣言しています。`xssh` と `xscp` は `openssh-client` と `sshpass`、`xftp` は `lftp`、`xsmb` は `smbclient`、`xgobuster` は `gobuster` と `wordlists`、`xffuf` は `ffuf` と `seclists` を使用します。
