...
依存パッケージは各Debianパッケージで宣言しています。`xssh` は `openssh-client` と `sshpass`、`xftp` は `lftp`、`xsmb` は `smbclient`、`xgobuster` は `gobuster` と `wordlists` を使用します。
