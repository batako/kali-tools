...
`--target`を省略するとPrimary Targetの探索結果を返します。同じURLを複数回観測している場合は1件にまとめ、response情報には最新の観測結果を使用し、`sources`にはそのURLを発見したすべてのツールを格納します。
...
| フィールド | 型 | 説明 |
|---|---|---|
| `discoveries` | objectの配列 | 選択したTargetの重複排除済みWeb探索結果。存在しなければ`[]` |
| `discoveries[].url` | string | 発見した完全なURL |
| `discoveries[].origin` | string | URLのschemeとauthority |
| `discoveries[].path` | string | originを基準としたpath |
| `discoveries[].status_code` | integer | 最新観測時のHTTP response status code |
| `discoveries[].content_length` | integer \| null | 取得できた場合のresponse body長 |
| `discoveries[].redirect_url` | string \| null | 取得できた場合のredirect先 |
| `discoveries[].sources` | stringの配列 | URLを発見したツール。名前順 |
| `discoveries[].last_seen` | string | 最新観測の保存時刻 |
