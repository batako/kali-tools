...
func TestCompletionScriptsIncludeAllConfigKeys(t *testing.T) {
	for _, shell := range []string{"zsh", "bash"} {
		...
		for _, want := range []string{"project.root", "web.directory.max-requests", "web.file.max-requests", "web.vhost.max-requests", "web.vhost.calibration-samples", "web.vhost.calibration-confidence", "password.max-requests", "dns.max-queries", "web.tls.verify"} {
			if !strings.Contains(script, want) {
