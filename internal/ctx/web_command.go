...
func runWeb(args []string, stdout io.Writer) error {
	...
	var err error
	var showHelp bool
	args, showHelp, err = resolveResourceCommand("web", args, []string{"ls", "clear"}, "", "ls")
	if err != nil {
	...
	if output.Format == apiFormatJSON {
		return runJSONEndpoint(stdout, "web", output.FormatVersion, func(version string) (any, error) {
			return webAPIData(version, targetName)
		})
	}
	...
	return WriteWebDiscoveryList(stdout, target, discoveries)
}
...
func webAPIData(version, targetName string) (any, error) {
	switch version {
	case "1.0":
		return webAPIDataV1_0(targetName)
	default:
		return nil, fmt.Errorf("unsupported web format version after resolution: %s", version)
	}
}

func webAPIDataV1_0(targetName string) (any, error) {
	workspace, target, err := webCommandContext(targetName)
	if err != nil {
		return nil, err
	}
